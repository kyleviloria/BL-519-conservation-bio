...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="legend1_color1" select="'rgb(0,51,102)'"/>
  <xsl:param name="legend1_color2" select="'rgb(0,26,51)'"/>

  <xsl:param name="legend2_color1" select="'rgb(179,112,0)'"/>
  <xsl:param name="legend2_color2" select="'rgb(90,56,0)'"/>

  <xsl:param name="legend3_color1" select="'rgb(85,82,204)'"/>
  <xsl:param name="legend3_color2" select="'rgb(43,41,102)'"/>

  <xsl:param name="legend4_color1" select="'rgb(188,204,51)'"/>
  <xsl:param name="legend4_color2" select="'rgb(89,102,26)'"/>

  <xsl:param name="legend5_color1" select="'rgb(115,251,29)'"/>
  <xsl:param name="legend5_color2" select="'rgb(58,126,15)'"/>

  <xsl:param name="legend6_color1" select="'rgb(19,15,180)'"/>
  <xsl:param name="legend6_color2" select="'rgb(10,8,90)'"/>

  <xsl:param name="legend7_color1" select="'rgb(215,72,29)'"/>
  <xsl:param name="legend7_color2" select="'rgb(108,36,15)'"/>

  <xsl:param name="legend8_color1" select="'rgb(64,127,126)'"/>
  <xsl:param name="legend8_color2" select="'rgb(32,64,63)'"/>

  <xsl:param name="legend_x1" select="0"/>
  <xsl:param name="legend_y1" select="0"/>
  <xsl:param name="legend_x2" select="0"/>
  <xsl:param name="legend_y2" select="1"/>
  ...
  <xsl:param name="grad3_color1" select="'rgb(192,192,192)'"/>
  <xsl:param name="grad3_color2" select="'rgb(255,255,255)'"/>

  <xsl:param name="grad4_color1" select="'rgb(255,255,255)'"/>
  <xsl:param name="grad4_color2" select="'rgb(192,192,192)'"/>
  ...
  <!-- area properties -->
  <xsl:param name="area_opacity" select="0.7" />
  <xsl:param name="area_stroke_width" select="1.0" />
  <xsl:param name="area_stroke_color" select="'rgb(0,0,0)'" />
  ...
  <xsl:template match="graph">
    ...
    <xsl:variable name="temp_width" select="exsl4D:max($viewport_width,(($data_group_width_min*($x_values_count - 1))+$graph_left_margin+$graph_right_margin))" />
    <xsl:variable name="bar_width" select="($bar_size*(1 - $bar_prop))+((((($temp_width - $graph_left_margin - $graph_right_margin) div ($x_values_count - 1)) - ($base_margin*4)))*$bar_prop)" />
    ...
    <xsl:variable name="bar_group_offset" select="0"/>

    <!-- viewbox properties -->
    <xsl:variable name="viewbox_width" select="($data_group_width*($x_values_count - 1))+$graph_left_margin+$graph_right_margin" />
    ...
    <svg id="ID_svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 {$viewbox_width} {$viewbox_height}" viewport-fill="{$viewport_color}" viewport-fill-opacity="{$viewport_opacity}">
      ...
      <defs >
        <xsl:call-template name="bar_gradient_defs">
          ...
          <xsl:with-param name="x1" select="$legend_x1"/>
          <xsl:with-param name="y1" select="$legend_y1"/>
          <xsl:with-param name="x2" select="$legend_x2"/>
          <xsl:with-param name="y2" select="$legend_y2"/>
        </xsl:call-template>
      ...
      <g id="ID_globals" style="fill:{$font_color};stroke:none;font-family:{$font_family};font-size:{$font_size}px;font-weight:{$font_weight};font-style:{$font_style};text-decoration:{$font_decoration}">
        ...
        <xsl:for-each select="settings/x_grid">
          <xsl:if test="text() = 'true'" >
            <xsl:variable name="count_v" select="count(../../x_values/v)" />
            <g id="ID_x_grid" stroke="{$x_grid_color}" stroke-width="{$x_grid_width}">
              <xsl:for-each select="../../x_values/v[position() &lt; $count_v]">
        ...
        <g id="ID_x_values" style="fill:{$x_label_color};stroke:none;font-family:{$x_label_font_family};font-size:{$x_label_font_size}px;font-weight:{$x_label_font_weight};font-style:{$x_label_font_style};text-decoration:{$x_label_font_decoration};filter:{$x_label_font_filter}">
          <xsl:for-each select="x_values/v">
            <xsl:variable name="pos"
              select="$graph_left_margin+($data_group_width*(position()-1))"/>
        ...
        <xsl:comment>draw lines</xsl:comment>
        ...
        <xsl:variable name="line_color" >
          <xsl:choose>
            <xsl:when test="$area_stroke_width &gt; 0">
              <xsl:value-of select="$area_stroke_color"/>
            </xsl:when>
            <xsl:otherwise>
        ...
        <!-- set graph clipping -->
        <g clip-path="url(#ID_graph_clip)" fill-opacity="{$area_opacity}" stroke="{$line_color}" stroke-width="{$area_stroke_width}">
         
          <!-- draw polyline -->
          <xsl:for-each select="y_values">
            <xsl:comment>draw polyline for data type <xsl:value-of select="string(position())"></xsl:value-of></xsl:comment>
            ...
            <xsl:variable name="points">
              <xsl:for-each select="s">
                ...
                <xsl:variable name="x"
                              select="$graph_left_margin+($data_group_width*($xpos - 1))"/>
                <xsl:variable name="y"
              ...
              <xsl:value-of select="$viewbox_width - $graph_right_margin"/>
              <xsl:value-of select="' '"/>
              <xsl:value-of select="$y0"/>
              <xsl:value-of select="' '"/>
              
              <xsl:value-of select="$graph_left_margin"/>
              <xsl:value-of select="' '"/>
              <xsl:value-of select="$y0"/>
            </xsl:variable>
            <polyline id="ID_polyline_{position()}" fill="url(#ID_bar_grad{position()})" points="{$points}"/>
          </xsl:for-each>
